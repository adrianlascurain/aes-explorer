...
export default class AesCipher{
    ...
    static lowNibbleMask: number = 0x0f;
    static highNibbleMask: number = 0xf0;

    static rcon: readonly number[] = [
        0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
    ];

    static readonly SBOX: SBox = [
    ...
    static MIX_COLUMNS_MATRIX: readonly number[][] = [
        [0x02, 0x03, 0x01, 0x01],
        [0x01, 0x02, 0x03, 0x01],
        [0x01, 0x01, 0x02, 0x03],
        [0x03, 0x01, 0x01, 0x02],
    ...
    static MIX_COLUMNS_INVERSE_MATRIX: readonly number[][] = [
        [0x0E, 0x0B, 0x0D, 0x09],
        [0x09, 0x0E, 0x0B, 0x0D],
        [0x0D, 0x09, 0x0E, 0x0B],
        [0x0B, 0x0D, 0x09, 0x0E]
    ...
    static subBytes(state: Matrix4x4, sBox: SBox): Matrix4x4 {
        return state.map(row => 
            row.map(byte => {
                const rowIndex = (byte & AesCipher.highNibbleMask) >> 4;
                const colIndex = byte & AesCipher.lowNibbleMask;
    ...
    static nonReducedGaloisMultiply(a: number, b: number): number[] {
        ...
        // Si alguno es 0, retornar [0]
        if (!a || !b) return [0];

        // Calcular el grado máximo: (bit más alto de a) + (bit más alto de b)
        const maxDegreeA = 31 - Math.clz32(a);
        const maxDegreeB = 31 - Math.clz32(b);
        const maxDegree = maxDegreeA + maxDegreeB;

        const result = Array(maxDegree + 1).fill(0);

        for (let i = 0; i <= maxDegreeB; i++) {
            if ((b >> i) & 1) {
                for (let j = 0; j <= maxDegreeA; j++) {
                    if ((a >> j) & 1) {
                        result[i + j] += 1;
                    }
                }
        ...
        // MSB at index maxDegree -> LSB at index 0
